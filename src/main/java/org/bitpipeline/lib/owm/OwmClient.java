...
public class OwmClient {
	 ...
	 * @throws JSONException if the response from the OWM server can't be parsed
	 * @throws IOException if there's some network error or the OWM server replies with a error. */
	public StatusWeatherData currentWeatherAtCity (long cityId) throws IOException, JSONException {
		String subUrl = String.format (Locale.ROOT, "weather/city/%d?type=json&units=metric", Long.valueOf (cityId));
		JSONObject response = doQuery (subUrl);
	 ...
	 * @throws JSONException if the response from the OWM server can't be parsed
	 * @throws IOException if there's some network error or the OWM server replies with a error. */
	public WeatherForecastResponse forecastWeatherAtCity (long cityId) throws JSONException, IOException {
